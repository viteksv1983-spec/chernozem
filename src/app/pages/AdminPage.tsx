...
function NInput({ value, onChange, min, max }: { value: number; onChange: (v: number) => void; min?: number; max?: number }) {
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Allow empty field during typing — don't commit NaN
    if (raw === "" || raw === "-") return;
    let num = Number(raw);
...
export function AdminPage() {
  ...
  const handleReset = async () => {
    ...
    const answer = prompt(
      "⚠️ Увага! Ця дія скине ВСЕ налаштування до початкових.\n\n" +
      "Для підтвердження введіть слово: СКИНУТИ"
    );
    if (answer?.trim() !== "СКИНУТИ") {
      if (answer !== null) setToast("Скидання скасовано — введено невірне слово");
      return;
    }
    resetContent();
    ...
    try {
      ...
      setToast("Скинуто до початкових налаштувань та збережено на сервері");
    } catch (e) {
            ...
            <>
              ...
              <Card title="🔐 Безпека — зміна пароля">
                <p style={{ fontSize: "13px", color: C.muted, marginBottom: "16px", lineHeight: 1.5 }}>
                  Для зміни пароля введіть поточний пароль та новий пароль нижче.
